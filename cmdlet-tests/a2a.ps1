...
$GLOBALS.currentTest = $DATA.Tests.A2A
...
$GLOBALS.writeCallHeader("A2A test Set up")
$newPolicyStr = '{
  "Name": "Linux Policy",
...
try {
   ...
   $GLOBALS.badResult(@{ minVerbosity = 0; cmd = "A2A general"; message = "Failed in A2A test setup"; ex = $_; })
   $ready =$false
   script:Cleanup
}
...
if($ready){
   try {
      $GLOBALS.writeCallHeader("Testing A2A Cmndlets")
      ...
      $GLOBALS.goodResult(@{ minVerbosity = 1; cmd = "New-SafeguardA2a"; message = "Success"; })
      Get-SafeguardA2A -A2aToGet $newA2ARegistration.Id  > $null
      $GLOBALS.goodResult(@{ minVerbosity = 1; cmd = "Get-SafeguardA2a"; message = "Success"; })
      Set-SafeguardA2aAccessRequestBroker -ParentA2a $newA2ARegistration.Id -Users $userUsername > $null
      $GLOBALS.goodResult(@{ minVerbosity = 1; cmd = "Set-SafeguardA2aAccessRequestBroker"; message = "Success"; })
      $compareRequestBroker = Get-SafeguardA2aAccessRequestBroker -ParentA2a $newA2ARegistration.Id
      $GLOBALS.goodResult(@{ minVerbosity = 1; cmd = "Get-SafeguardA2aAccessRequestBroker"; message = "Success"; })
      if ($null -ne $compareRequestBroker) {
         Get-SafeguardA2aAccessRequestBrokerApiKey -ParentA2a $newA2ARegistration.Id > $null
         $GLOBALS.goodResult(@{ minVerbosity = 1; cmd = "Get-SafeguardA2aAccessRequestBrokerApiKey"; message = "Success"; })
         Reset-SafeguardA2aAccessRequestBrokerApiKey -ParentA2a $newA2ARegistration.Id > $null
         $GLOBALS.goodResult(@{ minVerbosity = 1; cmd = "Reset-SafeguardA2aAccessRequestBrokerApiKey"; message = "Success"; })
      }
      else {
         Write-Host "Failed to get new A2A access request broker "
         $GLOBALS.badResult(@{ minVerbosity = 0; cmd = "Get-SafeguardA2aAccessRequestBroker"; message = "Couldn't find"; extra = $newA2ARegistration; })
      }
      $selectedAccount = FindAssetAccount(0)
      Add-SafeguardA2aCredentialRetrieval -ParentA2a $newA2ARegistration.Id -Account $selectedAccount.Id > $null
      $GLOBALS.goodResult(@{ minVerbosity = 1; cmd = "Add-SafeguardA2aCredentialRetrieval"; message = "Success"; })
      $compareCredentialRetrieval = Get-SafeguardA2aCredentialRetrieval -ParentA2a $newA2ARegistration.Id
      $GLOBALS.goodResult(@{ minVerbosity = 1; cmd = "Get-SafeguardA2aCredentialRetrieval"; message = "Success"; })
      if ($null -ne $compareCredentialRetrieval) {
         Get-SafeguardA2aCredentialRetrievalInformation -AccountName $cr.AccountName > $null
         $GLOBALS.goodResult(@{ minVerbosity = 1; cmd = "Get-SafeguardA2aCredentialRetrievalInformation"; message = "Success"; })
         foreach ($cr in $compareCredentialRetrieval) {
            ...
            $GLOBALS.goodResult(@{ minVerbosity = 1; cmd = "Get-SafeguardA2aCredentialRetrievalApiKey"; message = "Success"; })
            Reset-SafeguardA2aCredentialRetrievalApiKey -ParentA2a $newA2ARegistration.Id -Account $cr.AccountName > $null
            $GLOBALS.goodResult(@{ minVerbosity = 1; cmd = "Reset-SafeguardA2aCredentialRetrievalApiKey"; message = "Success"; })
         }
      }
      else {
         Write-Host "Failed to get the new A2A credential retrieval "
         $GLOBALS.badResult(@{ minVerbosity = 0; cmd = "Get-SafeguardA2aCredentialRetrieval"; message = "Couldn't find"; extra = $newA2ARegistration; })
      }
      Enable-SafeguardA2aService > $null
      $GLOBALS.goodResult(@{ minVerbosity = 1; cmd = "Enable-SafeguardA2aService"; message = "Success"; })
      #endregion
      Get-SafeguardA2aServiceStatus > $null
      $GLOBALS.goodResult(@{ minVerbosity = 1; cmd = "Get-SafeguardA2aServiceStatus"; message = "Success"; })
      ...
      if ($null -ne $a2aRegistrations) {
         ...
         $GLOBALS.goodResult(@{ minVerbosity = 1; cmd = "Get-SafeguardA2aAccessRequestBrokerIpRestriction"; message = "Success"; })
         ...
         if ($null -ne $a2aRetrieval) {
            ...
            $GLOBALS.goodResult(@{ minVerbosity = 1; cmd = "Get-SafeguardA2aCredentialRetrievalIpRestriction"; message = "Success"; })
         }
      }
      $script:completedSuccessfully = $true
   }
      ...
      $GLOBALS.badResult(@{ minVerbosity = 0; cmd = "A2A general"; message = "Unexpected error in A2A test"; ex = $_; })
   } 
   finally {
      if (!($script:completedSuccessfully -or $script:exceptionCaught)) {
         Write-Host
