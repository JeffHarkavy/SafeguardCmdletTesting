...
try {
   if ($DATA.requiredDNS -ne "") {
      ...
      if ($x0.DnsServers -contains $DATA.requiredDNS) {
         ...
         try {
            ...
            badResult "Set-SafeguardNetworkInterface" "Unexpected error setting DNS $DATA.requiredDNS on X0" $_.Exception
            throw $_.Exception
         }
      }
   }
   ...
   try {
      ...
      if ($_.Exception.Message -match "unable to find") {
         ...
         badResult "Get-SafeguardDirectory" "Unexpected error getting directory $($DATA.domainName)" $_.Exception
         throw $_.Exception
      }
   }
   ...
   try {
      ...
      badResult "Edit-SafeguardDirectory" "Unexpected error editing directory $($DATA.domainName)" $_.Exception
   }
   ...
   $existingAccounts = Get-SafeguardDirectoryAccount -DirectoryToGet $domainname -Fields Name
   ...
   foreach ($acctname in $DATA.directoryAccounts.GetEnumerator()) {
      ...
      if ($found) { infoResult "New-SafeguardDirectoryAccount" "$acctname already exists on $($DATA.domainName)" }
         ...
         try {
            ...
            badResult "New-SafeguardDirectoryAccount" "Unexpected error creating $acctName on $($DATA.domainName)" $_.Exception
         }
      }
   }
   ...
   $diracct = Get-SafeguardDirectoryAccount -DirectoryToGet $domainname -AccountToGet $DATA.directoryAccounts[0]
   ...
   try {
      ...
      badResult "Set-SafeguardDirectoryAccountPassword" "Set password failed on $($DATA.domainName)\$($diracct.Name)" $_.Exception
   }
   ...
   try {
      ...
      badresult "Add-SafeguardUserLinkedAccount" "$userUserName not created or available. Skipping LinkedAccount tests"
   }

   if ($newUser) {
      $linked = Add-SafeguardUserLinkedAccount -UserToSet $userUserName -DirectoryToAdd $DATA.domainName -AccountToAdd $DATA.directoryAccounts[0]
      if ($linked.Name -eq $DATA.directoryAccounts[0]) { goodResult "Add-SafeguardUserLinkedAccount" "Added Linked $($DATA.directoryAccounts[0]) account to $userUserName" }
      else { badResult "Add-SafeguardUserLinkedAccount" "Add linked account failed" }
      try {
         $linked = Get-SafeguardUserLinkedAccount -UserToGet $userUserName
         foreach ($acct in $linked) { goodResult "Get-SafeguardUserLinkedAccount" "User $userUserName linked account $($acct.Name)" }
      } catch {
         badResult "Get-SafeguardUserLinkedAccount" "Failed" $_.Exception
      }
      try {
         Remove-SafeguardUserLinkedAccount -UserToSet $userUserName -DirectoryToRemove $DATA.domainName -AccountToRemove $DATA.directoryAccounts[0] > $null
         goodResult "Remove-SafeguardUserLinkedAccount" "Successfully removed linked account from $userUserName"
      } catch {
         badResult "Remove-SafeguardUserLinkedAccount" "Failed" $_.Exception
      }
   }

   try {
      Remove-SafeguardDirectoryAccount -DirectoryToUse $domainname -AccountToDelete $DATA.directoryAccounts[2] > $null
      goodResult "Remove-SafeguardDirectoryAccount" "Successfully removed $($DATA.directoryAccounts[2]) from $($DATA.domainName)"
   } catch {
      badResult "Remove-SafeguardDirectoryAccount" "Failed" $_.Exception
   }

   try {
      $result = Remove-SafeguardDirectory -DirectoryToDelete $domainname
      goodResult "Remove-SafeguardDirectory" "Successfully removed directory $($DATA.domainName)"
   } catch {
      badResult "Remove-SafeguardDirectory" "Failed" $_.Exception
   }

} catch {
      badResult "Directory general" "Unexpected error in Directory test" $_.Exception
} finally {
